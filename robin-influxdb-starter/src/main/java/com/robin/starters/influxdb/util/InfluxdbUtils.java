...
import com.robin.starters.influxdb.core.anno.Aggregate;
import com.robin.starters.influxdb.core.anno.Count;
import com.robin.starters.influxdb.core.Query;
...
public final class InfluxdbUtils {
                    ...
                    for (List<Object> value : series.getValues()) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < fieldSize; i++) {
                                ...
                                Field field = null;
                                try {
                                    field = clazz.getDeclaredField(StringUtils.lineToHump(fieldName));
                            ...
                            if (series.getTags() != null && !series.getTags().isEmpty()) {
                                for (Map.Entry<String, String> entry : series.getTags().entrySet()) {
                                    Field field = null;
                                    try {
                                        field = clazz.getDeclaredField(StringUtils.lineToHump(entry.getKey()));
                                        field.setAccessible(true);
                                    } catch (NoSuchFieldException e) {
                ...
                if (column.tag()) {
                    ...
                    if (field.get(object) != null) {
                        if("time".equals(column.name())){
                            builder.time(DateTimeUtils.localDateTimeToInstant((LocalDateTime) field.get(object)).getEpochSecond(), TimeUnit.SECONDS);
                        } else {
    ...
    private static void setFieldValue(Class<?> type, List<Object> value, int i, Object obj, Field field) throws IllegalAccessException {
        if (type.equals(Long.class)) {
            ...
            field.set(obj, DateTimeUtils.instantToLocalDateTime(value.get(i).toString()));
        } else {
    ...
    public static <T> String getAggregateSelect(Class<T> clazz) {
        ...
        for (Field field : fields) {
            ...
            if (!ObjectUtils.isEmpty(aggregate)) {
                ...
                temp.append(Query.aggregate(aggregate.tag().getTag(), aggregate.value()));
                i++;
            }
        }
